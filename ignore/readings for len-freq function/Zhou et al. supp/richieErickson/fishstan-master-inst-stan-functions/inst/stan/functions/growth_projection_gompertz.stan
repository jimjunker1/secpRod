...
matrix calculate_site_projections_gompertaz_rng(
  matrix mu_beta_cor,
  vector mu_gamma,
  int n_fish,
  row_vector age,
  ...
  for(site in 1:n_sites){
    ...
    for( p in 1:n_proj){
      site_projections[ site, p] =
        normal_rng(l_inf[site] * exp( - exp(-1.0 * g_growth[site] *
...
vector calculate_hyper_projection_gompertaz_rng(
    matrix mu_beta_cor,
    vector mu_gamma,
    int n_fish,
    row_vector age,
  ...
  for( p in 1:n_proj){
    hyper_projection[p] = normal_rng(l_inf_bar *
      exp(- exp(-1.0 * g_growth_bar * (age_project[p] - t0_bar))),
      sigma_length);
  }
