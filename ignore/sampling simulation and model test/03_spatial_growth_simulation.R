...
init_cohort <- function(i, j, start_day) {
  ...
  k <- log(M_inf / initial_mass) / runif(N_init, 290, 310)
...
save(daily_sampling, file = here::here("vignettes/single_cohort_sim.RData"))
...
daily_sampling %>%
  unnest(mass_distribution) %>%
  ggplot(aes(x = mass_distribution, y = factor(day))) +
