...
#Define IGR production bootstrap function:
  igr.int.prod <- function(DATA, site, first.date, last.date, habitat, taxon, TEMP, wrap=F, LM.a, LM.b, LM.p.ash, g.a, g.b, g.c, g.d, min.growth=0, temp.corr, boot.num, GROWTH_df,...){
    ...
    if (length(TEMP) == length(t.int) & length(temp.corr) == length(t.int)){
      ...
      if (taxon == "Micropsectra sp." | taxon == "Eukiefferiella" | taxon == "Chaetocladius dentiforceps" | taxon == "Midge 3-2" | taxon == "Macropelopia" | taxon == "ST9 Midge 4" |
          taxon == "Midge 4" | taxon == "Midge 5" | taxon == "Orthocladius oblidens" | taxon == "Midge 7" | taxon == "Midge 8" | taxon == "Orthocladius" | taxon == "Thienemanniella sp." |
          taxon == "Midge indet."){
        for (d in 1:length(t)){
      ...
      }else if (taxon == "Tubificid 1" | taxon == "Simulium vittatum" | taxon == "Simulium spp." |taxon == "Ostracoda" | taxon == "Oribatid 1" | taxon == "Leech" | taxon == "Nematoda" | taxon == "Sperchon glandulosus" | taxon == "Lumb 3" | taxon == "Springtail" | taxon =="Clinocera stagnalis" |
               taxon == "Antocha sp." | taxon == "Oribatid 2" |taxon == "Ephydridae sp." | taxon == "Oligochaeta indet." | taxon == "Potamophylax cingulatus" | taxon == "Tub. 2" | taxon == "Lumbricidae"){
        for (d in 1:length(t)){
          date.sub = growth[which(growth$JULIAN == t[d]),]
          date.samps <- date.sub[sample(nrow(date.sub),1, replace = T),5:dim(date.sub)[2]]
          x = as.numeric(nrow(date.samps))
        ...
        if (taxon == "Micropsectra sp." | taxon == "Eukiefferiella" | taxon == "Chaetocladius dentiforceps" | taxon == "Midge 3-2" | taxon == "Macropelopia" | taxon == "ST9 Midge 4" |
          ...
          for (d in 1:length(t)){
            ...
            for (s in 1:length(sizes$mm)){
              ...
              int.P[f,s] <- int.B[f,s]*t.int[f]*(g.a + (g.b*log(sizes$AFDM[s]))+ (g.c * TEMP[f]))		#Calculate interval production (interval biomass * interval duration * growth rate for each size class on each date * temp correction factor for each date)
            }
              else {
                int.P[f,s] <- int.B[f,s]*t.int[f]*min.growth			#Calculate interval production (interval biomass * interval duration * minimum growth rate if growth equation gives a zero or negative value)
              }
