  ...
  igr.int.prod <- function(DATA, site, first.date, last.date, habitat, taxon, TEMP, wrap=F, LM.a, LM.b, LM.p.ash, g.a, g.b, g.c, g.d, min.growth=0, temp.corr, boot.num, GROWTH_df,...){
    ...
    if (length(TEMP) == length(t.int) & length(temp.corr) == length(t.int)){
        ...
        for (d in 1:length(t)){
          ...
          for (s in 1:length(sizes$mm)){
            for (f in 1:length(t.int)){
              if (!is.null(x) && x != 0 && !is.na(date.samps[1,s])){
                int.P[f,s] = int.B[f,s]*t.int[f]*date.samps[1,s]
              } else if((g.a + (g.b*TEMP[f]) + (g.c * sizes$AFDM[s])) > 0){
              int.P[f,s] <- int.B[f,s]*t.int[f]*(exp(g.a + (g.b*sizes$AFDM[s]) + (g.c * TEMP[f])))		#Calculate interval production (interval biomass * interval duration * growth rate for each size class on each date * temp correction factor for each date)
            }
            else {
              int.P[f,s] <- int.B[f,s]*t.int[f]*min.growth			#Calculate interval production (interval biomass * interval duration * minimum growth rate if growth equation gives a zero or negative value)
      ...
      for (b in 1:boot.num){
        ...
        }else if (taxon == "Tub. 1" | taxon == "Tub. 2"| taxon == "Simulium vittatum" | taxon == "Simulium indet" | taxon == "Oribatid 1" | taxon == "Leech" | taxon == "Nematode" | taxon == "Sperchon glandulosus" | taxon == "Lumb 3" | taxon == "Springtail" | taxon =="Clinocera stagnalis" |
                 taxon == "Antocha sp." | taxon == "Oribatid 2" |taxon == "Ephydridae sp." | taxon == "Oligochaeta indet." | taxon == "Potamophylax cingulatus" | taxon == "Tub. 2" | taxon == "Prosimulium ursinum" | taxon == "Simulium vernum" | taxon == "Simulium aureum" | taxon == "Limnephilus sparsus" |
                 taxon == "Hirudinea" | taxon == "Limonia" | taxon == "Collembola" | taxon == "Oligochaeta A" | taxon == "Oligochaeta B" | taxon == "Oligochaeta C" | taxon == "Predatory Oligochaeta" | taxon == "Capnia vidua" | taxon == "Dytiscidae Larva" | taxon == "Dytiscidae Adult"){
          for (d in 1:length(t)){
            date.sub = growth[which(growth$JULIAN == t[d]),]
