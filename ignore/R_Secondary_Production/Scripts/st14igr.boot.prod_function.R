  ...
  igr.int.prod <- function(DATA, GROWTH_df, site, first.date, last.date, habitat, taxon, TEMP, wrap=F, LM.a, LM.b, LM.p.ash, g.a, g.b, g.c, g.d, min.growth=0.001, temp.corr, boot.num){
    ...
    if (length(TEMP) == length(t.int) & length(temp.corr) == length(t.int)){
      ...
      if(taxon == "Sperchon glandulosus"){
        AFDM <- exp(exp(LM.a)+LM.b*log(mm))*(1-(LM.p.ash/100))
      } else{
      ...
      } else if (taxon == "Tub. 1" | taxon == "Tub. 2"| taxon == "Simulium vittatum" | taxon == "Simulium indet" |taxon == "Ostracoda" | taxon == "Oribatid 1" | taxon == "Leech" | taxon == "Nematode" | taxon == "Sperchon glandulosus" | taxon == "Lumb 3" | taxon == "Springtail" | taxon =="Clinocera stagnalis" |
               taxon == "Antocha sp." | taxon == "Oribatid 2" |taxon == "Ephydridae sp." | taxon == "Oligochaeta indet." | taxon == "Potamaphylax cingulatus" | taxon == "Tub. 2" | taxon == "Prosimulium ursinum" | taxon == "Simulium vernum" | taxon == "Simulium aurem" | taxon == "Oribatid 3" | taxon == "Red Mite"){
        for (d in 1:length(t)){
          date.sub = growth[which(growth$JULIAN == t[d]),]
          date.samps <- date.sub[sample(nrow(date.sub),1, replace = T),6:dim(date.sub)[2]]
          x = as.numeric(nrow(date.samps))
            ...
            for (f in 1:length(t.int)){
              if (!is.null(x) && x != 0 && !is.na(date.samps)){
                int.P[f,s] = int.B[f,s]*t.int[f]*date.samps[s]
                    ...
                    int.P[f,s] <- int.B[f,s]*t.int[f]*(g.a + (g.b*overkt_to_C(TEMP[f])))		#Calculate interval production (interval biomass * interval duration * growth rate for each size class on each date * temp correction factor for each date)
                  }
                  else {
                    int.P[f,s] <- int.B[f,s]*t.int[f]*min.growth			#Calculate interval production (interval biomass * interval duration * minimum growth rate if growth equation gives a zero or negative value)
                  }
