...
#Define IGR production bootstrap function:
  igr.int.prod <- function(DATA, site, first.date, last.date, habitat, taxon, TEMP, wrap=F, LM.a, LM.b, LM.p.ash, g.a, g.b, g.c, g.d, min.growth=0, temp.corr, boot.num, GROWTH_df,...){
    ...
    if (length(TEMP) == length(t.int) & length(temp.corr) == length(t.int)){
      ...
      if (taxon == "Midge 1" | taxon == "Midge 2" | taxon == "Midge 3" | taxon == "Midge 3-2" | taxon == "Tanypodinae" | taxon == "ST9 Midge 4" | taxon == "O. Fridgidus" | taxon == "Ceratopegonid" |
        ...
        for (d in 1:length(t)){
          ...
          date.samps <- date.sub[sample(nrow(date.sub),1, replace = T),6:dim(date.sub)[2]]
          x = as.numeric(nrow(date.samps))
          for (s in 1:length(sizes$mm)){
            for (f in 1:length(t.int)){
              if (!is.null(x) && x != 0 && !is.na(date.samps)){
                int.P[f,s] = int.B[f,s]*t.int[f]*date.samps[s]
              } else if((g.a-(g.b*log(sizes$mm[s]))+(g.c*TEMP[f])) > 0){
              int.P[f,s] <- int.B[f,s]*t.int[f]*(g.a-(g.b*log(sizes$mm[s]))+(g.c*TEMP[f]))	#Calculate interval production (interval biomass * interval duration * growth rate for each size class and temp on each date)
            }
            else {
        ...
        for (d in 1:length(t)){
          ...
          for (s in 1:length(sizes$mm)){
            for (f in 1:length(t.int)){
              ...
              int.P[f,s] <- int.B[f,s]*t.int[f]*(exp(g.a + (g.b*TEMP[d]) + (g.c*log(sizes$AFDM[s]))))	#Calculate interval production (interval biomass * interval duration * growth rate for each size class on each date * temp correction factor for each date)
            }
            else {
              int.P[f,s] <- int.B[f,s]*t.int[f]*min.growth			#Calculate interval production (interval biomass * interval duration * minimum growth rate if growth equation gives a zero or negative value)
            }
               ...
               taxon == "Antocha sp." | taxon == "Oribatid 2" | taxon == "Oribatid 3" |taxon == "Ephydridae sp." | taxon == "Oligochaeta indet." | taxon == "Potamophylax" | taxon == "Tub. 2" | taxon == "Prosimulium" | taxon == "S. vernum" | taxon == "S. aureum"){
        for (d in 1:length(t)){
          date.sub = growth[which(growth$JULIAN == t[d]),]
          date.samps <- date.sub[sample(nrow(date.sub),1, replace = T),6:dim(date.sub)[2]]
          x = as.numeric(nrow(date.samps))
          for (s in 1:length(sizes$mm)){
            for (f in 1:length(t.int)){
              ...
              int.P[f,s] <- int.B[f,s]*t.int[f]*(g.a + (g.b*TEMP[f]) + (g.c * sizes$AFDM[s]))		#Calculate interval production (interval biomass * interval duration * growth rate for each size class on each date * temp correction factor for each date)
            }
            else {
              int.P[f,s] <- int.B[f,s]*t.int[f]*min.growth			#Calculate interval production (interval biomass * interval duration * minimum growth rate if growth equation gives a zero or negative value)
            }
        ...
        for (d in 1:length(t)){
          ...
          for (s in 1:length(sizes$mm)){
            for (f in 1:length(t.int)){
              if (!is.null(x) && x != 0 && !is.na(date.samps)){
                int.P[f,s] = int.B[f,s]*t.int[f]*date.samps[s]
              } else if((g.a + (g.b*TEMP[f])) > 0){
                    int.P[f,s] <- int.B[f,s]*t.int[f]*(g.a + (g.b*TEMP[f]))		#Calculate interval production (interval biomass * interval duration * growth rate for each size class on each date * temp correction factor for each date)
                  }
                  else {
                    int.P[f,s] <- int.B[f,s]*t.int[f]*min.growth			#Calculate interval production (interval biomass * interval duration * minimum growth rate if growth equation gives a zero or negative value)
                  }
        ...
        for (d in 1:length(t)){
          ...
          for (s in 1:length(sizes$mm)){
            for (f in 1:length(t.int)){
              ...
              int.P[f,s] <- int.B[f,s]*t.int[f]*(exp(g.a + (g.b*TEMP[f]) + (g.c*log(sizes$AFDM[s]))))	#Calculate interval production (interval biomass * interval duration * growth rate for each size class on each date * temp correction factor for each date)
            }
              else {
                int.P[f,s] <- int.B[f,s]*t.int[f]*min.growth		#Calculate interval production (interval biomass * interval duration * minimum growth rate if growth equation gives a zero or negative value)
              }
