...
#Define a wrapper function to run production calcs on all taxa and all habitats for a given site and year:
  wrapper.site.CED.int <- function(DATA, site, first.date, last.date, habitat, TAXA, TEMP.COB, TEMP.DEP, TEMP.TAL, wrap=F, temp.corr.igr.cob, temp.corr.igr.dep, temp.corr.igr.tal, temp.corr.sfpb=1, boot.num,...){
    ...
    Nboots.tal <- Nboots.dep <- Nboots.cob <- Bboots.tal <- Bboots.dep <- Bboots.cob <- CEDboots.tal <- CEDboots.dep <- CEDboots.cob <- data.frame(matrix(NA, nrow=boot.num, ncol=length(taxa.levels)))
    CEDintboots.cob <- data.frame(matrix(NA, nrow = length(taxa.levels), ncol = num.intervals))
    ...
    for (a in 1:length(taxa.levels)){
      ...
      if (is.na(tax.info$METHOD)){
        ...
        out <- out.cob <- out.dep <- out.tal <- data.frame(CEDboots=rep(NA, boot.num), Bboots=rep(NA, boot.num), Nboots=rep(NA, boot.num))
      } else if (tax.info$METHOD == "igr"){
        if (habitat == "COBBLE"){
          out <- int.CED(DATA=DATA, site=site, first.date=first.date, last.date=last.date, habitat=habitat,
          ...
          sink(paste("Output/CED", site, "_", gsub("/", "-", first.date), "_", gsub("/", "-", last.date), "_", habitat, "_", taxa.levels[a], ".txt", sep=""))
          print(out)
          sink()
        }
        ...
        if (habitat == "ALL"){
          out.cob <- int.CED(DATA=DATA, site=site, first.date=first.date, last.date=last.date, habitat="COBBLE",
                       taxon=as.character(tax.info$TAXON), TEMP=TEMP.COB, wrap=wrap,
                       LM.a=tax.info$LM.a, LM.b=tax.info$LM.b, LM.p.ash=tax.info$LM.p.ash,
                       g.a=tax.info$g.a, g.b=tax.info$g.b, g.c=tax.info$g.c, g.d=tax.info$g.d, min.growth=tax.info$min.growth,
                       temp.corr=temp.corr.igr.cob, boot.num=boot.num)
          ...
          print(out.cob)
          sink()
          out.dep <- int.CED(DATA=DATA, site=site, first.date=first.date, last.date=last.date, habitat="DEPOSITIONAL",
                       taxon=as.character(tax.info$TAXON), TEMP=TEMP.DEP, wrap=wrap,
                       LM.a=tax.info$LM.a, LM.b=tax.info$LM.b, LM.p.ash=tax.info$LM.p.ash,
                       g.a=tax.info$g.a, g.b=tax.info$g.b, g.c=tax.info$g.c, g.d=tax.info$g.d, min.growth=tax.info$min.growth,
                       temp.corr=temp.corr.igr.dep, boot.num=boot.num)
          ...
          print(out.dep)
          sink()
          out.tal <- int.CED(DATA=DATA, site=site, first.date=first.date, last.date=last.date, habitat="TALUS.CLIFF",
                       taxon=as.character(tax.info$TAXON), TEMP=TEMP.TAL, wrap=wrap,
                       LM.a=tax.info$LM.a, LM.b=tax.info$LM.b, LM.p.ash=tax.info$LM.p.ash,
                       g.a=tax.info$g.a, g.b=tax.info$g.b, g.c=tax.info$g.c, g.d=tax.info$g.d, min.growth=tax.info$min.growth,
                       temp.corr=temp.corr.igr.tal, boot.num=boot.num)
      ...
      if (habitat == "COBBLE" & dim(tax.info)[1] != 0){
        CEDboots.cob[,a] <- out$CEDboots
        names(CEDboots.cob)[a] <- as.character(tax.info$TAXON)
        ...
        #CEDintboots.out[which(CEDintboots.out == 0)] = NA
        #browser()
        rownames(Nintboots.cob)[a] = as.character(tax.info$TAXON)
        CEDintboots.out <- out$CEDintboots
        CEDintboots.out[is.na(CEDintboots.out) | is.null(CEDintboots.out)] <- 0
        if (is.null(CEDintboots.out) |all(is.na(CEDintboots.out)) | all(is.nan(CEDintboots.out))){
          CEDintboots.cob[a,] <- 0
        }  
        else if(all(CEDintboots.out == 0)){
          CEDintboots.cob[a,] <- 0
        } 
        else {
          CEDintboots.cob[a,] = apply(data.frame(CEDintboots.out), 2, mean, na.rm = T)
        } 
        #CEDintboots.out[which(CEDintboots.out == 0)] = NA
        rownames(CEDintboots.cob)[a] = as.character(tax.info$TAXON)
        ...
        CEDboots.dep[,a] <- out$CEDboots
        names(CEDboots.dep)[a] <- as.character(tax.info$TAXON)
        ...
        CEDboots.tal[,a] <- out$CEDboots
        names(CEDboots.tal)[a] <- as.character(tax.info$TAXON)
        ...
        CEDboots.cob[,a] <- out.cob$CEDboots
        CEDboots.dep[,a] <- out.dep$CEDboots
        CEDboots.tal[,a] <- out.tal$CEDboots
        ...
        names(CEDboots.cob)[a] <- names(CEDboots.dep)[a] <- names(CEDboots.tal)[a] <- names(Bboots.cob)[a] <- names(Bboots.dep)[a] <- names(Bboots.tal)[a] <- names(Nboots.cob)[a] <- names(Nboots.dep)[a] <- names(Nboots.tal)[a] <- as.character(tax.info$TAXON)
      }
    }
    list(CEDboots.cob=CEDboots.cob, CEDboots.dep=CEDboots.dep, CEDboots.tal=CEDboots.tal, Bboots.cob=Bboots.cob, Bboots.dep=Bboots.dep, Bboots.tal=Bboots.tal, Nboots.cob=Nboots.cob, Nboots.dep=Nboots.dep, Nboots.tal=Nboots.tal, Nintboots.cob = Nintboots.cob, CEDintboots.cob = CEDintboots.cob, Bintboots.cob = Bintboots.cob)
    }
 }
