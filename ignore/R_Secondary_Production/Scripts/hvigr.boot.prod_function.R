...
print(taxon)
      ...
      if (taxon == "Midge 1" | taxon == "Midge 2"){
        for (d in 1:length(t)){
          ...
          for (f in 1:length(t.int)){
            if (!is.null(x) && x != 0 && !is.na(date.samps)){
              ...
              int.P[f,s] <- int.B[f,s]*t.int[f]*(g.a-(g.b*log(sizes$mm[s]))+(g.c*TEMP[d]))	#Calculate interval production (interval biomass * interval duration * growth rate for each size class and temp on each date)
            }  else {
      ...
      } else if (taxon == "Tub. 1" | taxon == "S. vittatum" | taxon == "Ostracod" | taxon == "Oribatid 1" | taxon == "Leech" | taxon == "Nematode" | taxon == "Sperchon sp." | taxon == "Lumb 3" | taxon == "Springtail" | taxon =="Clinocera"){
        for (d in 1:length(t)){
          date.sub = growth[which(growth$JULIAN == t[d]),]
          date.samps <- date.sub[sample(nrow(date.sub),1, replace = T),6:dim(date.sub)[2]]
          x = as.numeric(nrow(date.samps))
      ...
      list(totdays=sum(t.int), Psampest=Psampest, Pbootest=c(Mean=mean(Pboots), quantile(Pboots, c(0.025, 0.5, 0.975))), Pboots=Pboots, Bboots=Bboots, Nboots=Nboots, julians=t, intdays=t.int, Pintsampest=Pintsampest, Pintboots=Pintboots, Bdatesampinfo=Bdatesampinfo, Ndatesampinfo=Ndatesampinfo, Bdateboots=Bdateboots, Ndateboots=Ndateboots, Nintboots = Nintboots)
    }
