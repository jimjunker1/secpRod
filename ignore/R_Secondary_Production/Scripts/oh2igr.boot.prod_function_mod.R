...
#Define IGR production bootstrap function:
  igr.int.prod <- function(DATA, site, first.date, last.date, habitat, taxon, TEMP, wrap=F, LM.a, LM.b, LM.p.ash, g.a, g.b, g.c, g.d, min.growth=0, temp.corr, boot.num, GROWTH_df,...){
    ...
    growth <- GROWTH_df[GROWTH_df$SITE == site & GROWTH_df$TAXON == taxon,]
    ...
    if (length(TEMP) == length(t.int) & length(temp.corr) == length(t.int)){
      ...
      if(taxon == "Sperchon glandulosus"){
        AFDM <- exp(LM.a+LM.b*log(mm))*(1-(LM.p.ash/100))
      }else if(taxon == "Ostracoda"){
        AFDM <- ((LM.a+mm^LM.b)*0.108)*(1-(LM.p.ash/100))
      ...
      if (taxon == "Ceratopogonidae A" | taxon == "Ceratopogonidae B" | taxon == "Macropelopia" | taxon == "Eukiefferiella minor" | taxon == "Eukiefferiella claripennis" | taxon == "Diamesa bohemani_zernyi" |
            taxon == "Diamesa bertrami" | taxon == "Micropsectra sp." | taxon == "Parochlus" | taxon == "Thienemanniella sp." | taxon == "Thienemaniella sp."|taxon == "Orthocladius frigidus" | taxon == "Orthocladius oblidens" | taxon == "Metriocnemus" |
          taxon == "Rheocricotopus effusus" | taxon == "Chaetocladius" |taxon == "Limnophyes" | taxon == "Thaumaleidae"){
        for (d in 1:length(t)){
          date.sub = growth[which(growth$JULIAN == t[d]),]
          date.samps <- date.sub[sample(nrow(date.sub),1, replace = T),5:dim(date.sub)[2]]
          x = as.numeric(nrow(date.samps))
          for (s in 1:length(sizes$mm)){
            for (f in 1:length(t.int)){
              if (!is.null(x) && x != 0 && !is.na(date.samps[1,s])){
                int.P[f,s] = int.B[f,s]*t.int[f]*date.samps[1,s]
              } else if((g.a-(g.b*log(sizes$mm[s]))+(g.c*TEMP[f])) > 0){
              int.P[f,s] <- int.B[f,s]*t.int[f]*(g.a-(g.b*log(sizes$mm[s]))+(g.c*overkt_to_C(TEMP[f])))	#Calculate interval production (interval biomass * interval duration * growth rate for each size class and temp on each date)
            }
            else {
              int.P[f,s] <- int.B[f,s]*t.int[f]*min.growth	#Calculate interval production (interval biomass * interval duration * minimum growth rate if growth equation gives a zero or negative value)
            }
          }
        }
      }
        ...
        for (d in 1:length(t)){
          ...
          for (s in 1:length(sizes$mm)){
            for (f in 1:length(t.int)){
              if (!is.null(x) && x != 0 && !is.na(date.samps[1,s])){
                int.P[f,s] = int.B[f,s]*t.int[f]*date.samps[1,s]
              } else if((g.a + (g.b*log(sizes$AFDM[s]))+ (g.c * TEMP[f])) > 0){
              int.P[f,s] <- int.B[f,s]*t.int[f]*(exp(g.a + (g.b*sizes$AFDM[s]) + (g.c * TEMP[f])))		#Calculate interval production (interval biomass * interval duration * growth rate for each size class on each date * temp correction factor for each date)
            }
            else {
              int.P[f,s] <- int.B[f,s]*t.int[f]*min.growth	#Calculate interval production (interval biomass * interval duration * minimum growth rate if growth equation gives a zero or negative value)
            }
        ...
        for (d in 1:length(t)){
          ...
          for (s in 1:length(sizes$mm)){
            for (f in 1:length(t.int)){
              if (!is.null(x) && x != 0 && !is.na(date.samps[1,s])){
                int.P[f,s] = int.B[f,s]*t.int[f]*date.samps[1,s]
              } else if(exp(g.a + (g.b*sizes$AFDM[s]) + (g.c * TEMP[d])) > 0 |!is.na(exp(g.a + (g.b*sizes$AFDM[s]) + (g.c * TEMP[d])))){
              int.P[f,s] <- int.B[f,s]*t.int[f]*(exp(g.a + (g.b*sizes$AFDM[s]) + (g.c * TEMP[d])))	#Calculate interval production (interval biomass * interval duration * growth rate for each size class on each date * temp correction factor for each date)
            }
            else {
              int.P[f,s] <- int.B[f,s]*t.int[f]*min.growth			#Calculate interval production (interval biomass * interval duration * minimum growth rate if growth equation gives a zero or negative value)
            }
      ...
      }else if (taxon == "Tub. 1" | taxon == "Tub. 2"| taxon == "Simulium vittatum" | taxon == "Simulium indet" | taxon == "Oribatid 1" | taxon == "Leech" | taxon == "Nematoda" | taxon == "Sperchon glandulosus" | taxon == "Lumb 3" | taxon == "Springtail" | taxon =="Clinocera stagnalis" |
               taxon == "Antocha sp." | taxon == "Oribatid 2" |taxon == "Ephydridae sp." | taxon == "Oligochaeta indet." | taxon == "Potamophylax cingulatus" | taxon == "Tub. 2" | taxon == "Prosimulium ursinum" | taxon == "Simulium vernum" | taxon == "Simulium aureum" | taxon == "Limnephilus sparsus" |
        ...
        for (d in 1:length(t)){
          ...
          for (s in 1:length(sizes$mm)){
            for (f in 1:length(t.int)){
              if (!is.null(x) && x != 0 && !is.na(date.samps[1,s])){
                int.P[f,s] = int.B[f,s]*t.int[f]*date.samps[1,s]
              } else if(exp(g.a + (g.b*sizes$AFDM[s]) + (g.c * TEMP[f])) > 0){
              int.P[f,s] <- int.B[f,s]*t.int[f]*(exp(g.a + (g.b*sizes$AFDM[s]) + (g.c * TEMP[f])))		#Calculate interval production (interval biomass * interval duration * growth rate for each size class on each date * temp correction factor for each date)
            }
            else {
              int.P[f,s] <- int.B[f,s]*t.int[f]*min.growth			#Calculate interval production (interval biomass * interval duration * minimum growth rate if growth equation gives a zero or negative value)
              ...
              for (d in 1:length(t)){
                ...
                for (s in 1:length(sizes$mm)){
                  for (f in 1:length(t.int)){
                    if (!is.null(x) && x != 0 && !is.na(date.samps[1,s])){
                      int.P[f,s] = int.B[f,s]*t.int[f]*date.samps[1,s]
                    } else if((exp(g.a + (g.b*log(sizes$AFDM[s])) + (g.c * TEMP[f]))) > 0){
              int.P[f,s] <- int.B[f,s]*t.int[f]*(exp(g.a + (g.b*sizes$AFDM[s]) + (g.c * overkt_to_C(TEMP[f]))))			#Calculate interval production (interval biomass * interval duration * growth rate for each size class on each date * temp correction factor for each date)
            }
            else {
              int.P[f,s] <- int.B[f,s]*t.int[f]*min.growth			#Calculate interval production (interval biomass * interval duration * minimum growth rate if growth equation gives a zero or negative value)
            }
        ...
        for (d in 1:length(t)){
          ...
          for (s in 1:length(sizes$mm)){
            for (f in 1:length(t.int)){
              if (!is.null(x) && x != 0 && !is.na(date.samps[1,s])){
                int.P[f,s] = int.B[f,s]*t.int[f]*date.samps[1,s]
              } else if(10^(g.a  + (g.b*log10(sizes$AFDM[s])) + (g.c * overkt_to_C(TEMP[f]))) > 0){
                int.P[f,s] <- int.B[f,s]*t.int[f]*(10^(g.a + (g.b*log10(sizes$AFDM[s])) + (g.c * overkt_to_C(TEMP[f]))))			#Calculate interval production (interval biomass * interval duration * growth rate for each size class on each date * temp correction factor for each date)
              }
              else {
                int.P[f,s] <- int.B[f,s]*t.int[f]*min.growth			#Calculate interval production (interval biomass * interval duration * minimum growth rate if growth equation gives a zero or negative value)
              }
      ...
      for (b in 1:boot.num){
        ...
        int.P <- matrix(0, length(t.int), length(sizes$mm))
        if (taxon == "Ceratopogonidae A" | taxon == "Ceratopegonidae B" | taxon == "Macropelopia" | taxon == "Eukiefferiella minor" | taxon == "Eukiefferiella claripennis" | taxon == "Diamesa bohemani_zernyi" |
            taxon == "Diamesa bertrami" | taxon == "Micropsectra sp." | taxon == "Parochlus" | taxon == "Thienemanniella sp."| taxon == "Orthocladius frigidus" | taxon == "Orthocladius oblidens" | taxon == "Metriocnemus" |
            taxon == "Rheocricotopus effusus" | taxon == "Chaetocladius" |taxon == "Limnophyes" | taxon == "Thaumaleidae"){
          for (d in 1:length(t)){
            date.sub = growth[which(growth$JULIAN == t[d]),]
            date.samps <- date.sub[sample(nrow(date.sub),1, replace = T),5:dim(date.sub)[2]]
            ...
            for (s in 1:length(sizes$mm)){
              for (f in 1:length(t.int)){
                if (!is.null(x) && x != 0 && !is.na(date.samps[1,s])){
                  int.P[f,s] = int.B[f,s]*t.int[f]*date.samps[1,s]
                } else if((g.a-(g.b*log(sizes$mm[s]))+(g.c*overkt_to_C(TEMP[f]))) > 0){
              int.P[f,s] <- int.B[f,s]*t.int[f]*(g.a-(g.b*log(sizes$mm[s]))+(g.c*overkt_to_C(TEMP[f])))	#Calculate interval production (interval biomass * interval duration * growth rate for each size class and temp on each date)
            }
              else {
                int.P[f,s] <- int.B[f,s]*t.int[f]*min.growth			#Calculate interval production (interval biomass * interval duration * minimum growth rate if growth equation gives a zero or negative value)
        ...
        for (d in 1:length(t)){
          ...
          for (s in 1:length(sizes$mm)){
            for (f in 1:length(t.int)){
              if (!is.null(x) && x != 0 && !is.na(date.samps[1,s])){
                int.P[f,s] = int.B[f,s]*t.int[f]*date.samps[1,s]
              } else if(exp(g.a + (g.b*TEMP[f]) + (g.c*log(sizes$AFDM[s]))) > 0 | !is.na(exp(g.a + (g.b*TEMP[f]) + (g.c*log(sizes$AFDM[s]))))){
              int.P[f,s] <- int.B[f,s]*t.int[f]*(exp(g.a + (g.b*sizes$AFDM[s]) + (g.c * TEMP[f])))		#Calculate interval production (interval biomass * interval duration * growth rate for each size class on each date * temp correction factor for each date)
            }
              else {
                int.P[f,s] <- int.B[f,s]*t.int[f]*min.growth		#Calculate interval production (interval biomass * interval duration * minimum growth rate if growth equation gives a zero or negative value)
        ...
        }else if (taxon == "Tub. 1" | taxon == "Tub. 2"| taxon == "Simulium vittatum" | taxon == "S. indet" | taxon == "Oribatid 1" | taxon == "Leech" | taxon == "Nematoda" | taxon == "Sperchon glandulosus" | taxon == "Lumb 3" | taxon == "Springtail" | taxon =="Clinocera stagnalis" |
                 taxon == "Antocha sp." | taxon == "Oribatid 2" |taxon == "Ephydridae sp." | taxon == "Oligochaeta indet." | taxon == "Potamophylax cingulatus" | taxon == "Tub. 2" | taxon == "Prosimulium ursinum" | taxon == "Simulium vernum" | taxon == "Simulium aureum" | taxon == "Limnephilus sparsus" |
                 taxon == "Hirudinea" | taxon == "Limonia" | taxon == "Collembola" | taxon == "Oligochaeta A" | taxon == "Oligochaeta B" | taxon == "Oligochaeta C" | taxon == "Predatory Oligochaeta" | taxon == "Capnia vidua" | taxon == "Dytiscidae Larva" | taxon == "Dytiscidae Adult"){
          for (d in 1:length(t)){
            date.sub = growth[which(growth$JULIAN == t[d]),]
                     ...
                     for (d in 1:length(t)){
                       ...
                       for (s in 1:length(sizes$mm)){
                         for (f in 1:length(t.int)){
                           if (!is.null(x) && x != 0 && !is.na(date.samps[1,s])){
                             int.P[f,s] = int.B[f,s]*t.int[f]*date.samps[1,s]
                           } else if((exp(g.a + (g.b*log(sizes$AFDM[s])) + (g.c * overkt_to_C(TEMP[f])))) > 0){
              int.P[f,s] <- int.B[f,s]*t.int[f]*(exp(g.a + (g.b*log(sizes$AFDM[s])) + (g.c * overkt_to_C(TEMP[f]))))	#Calculate interval production (interval biomass * interval duration * growth rate for each size class on each date * temp correction factor for each date)
            }
              else {
                int.P[f,s] <- int.B[f,s]*t.int[f]*min.growth		#Calculate interval production (interval biomass * interval duration * minimum growth rate if growth equation gives a zero or negative value)
              }
      ...
      list(totdays=sum(t.int), Psampest=Psampest, Pbootest=c(Mean=mean(Pboots), quantile(Pboots, c(0.025, 0.5, 0.975))), Pboots=Pboots, Bboots=Bboots, Nboots=Nboots, julians=t, intdays=t.int, Pintsampest=Pintsampest, Pintboots=Pintboots, Bdatesampinfo=Bdatesampinfo, Ndatesampinfo=Ndatesampinfo, Bintboots = Bintboots, Nintboots = Nintboots, Bdateboots=Bdateboots, Ndateboots=Ndateboots)
    }
