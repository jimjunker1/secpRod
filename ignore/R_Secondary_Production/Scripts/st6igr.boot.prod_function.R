  ...
  igr.int.prod <- function(DATA, site, first.date, last.date, habitat, taxon, TEMP, wrap=F, LM.a, LM.b, LM.p.ash, g.a, g.b, g.c, g.d, min.growth=0, temp.corr, boot.num, GROWTH_df,...){
    ...
    if (length(TEMP) == length(t.int) & length(temp.corr) == length(t.int)){
      ...
      if (taxon == "Midge 1" | taxon == "Midge 2" | taxon == "Midge 3" | taxon == "Midge 3-2" | taxon == "Tanypodinae" | taxon == "ST9 Midge 4" |
          taxon == "Midge 4" | taxon == "Midge 5" | taxon == "Midge 6" | taxon == "Midge 7" | taxon == "Midge 8" | taxon == "Orthocladius" | taxon == "Thienemanniella sp."){
        for (d in 1:length(t)){
        ...
        for (d in 1:length(t)){
          ...
          for (s in 1:length(sizes$mm)){
            for (f in 1:length(t.int)){
              if (!is.null(x) && x != 0 && !is.na(date.samps)){
                int.P[f,s] = int.B[f,s]*t.int[f]*date.samps[s]
              } else if(exp(g.a + (g.b*TEMP[d]) + (g.c*log(sizes$AFDM[s]))) > 0 |!is.na(exp(g.a + (g.b*TEMP[f]) + (g.c*log(sizes$AFDM[s]))))){
      ...
      }else if (taxon == "Tub. 1" | taxon == "S. vittatum" | taxon == "S. indet" |taxon == "Ostracod" | taxon == "Oribatid 1" | taxon == "Leech" | taxon == "Nematode" | taxon == "Sperchon sp." | taxon == "Lumb 3" | taxon == "Springtail" | taxon =="Clinocera" |
               taxon == "Antocha sp." | taxon == "Oribatid 2" |taxon == "Ephydridae sp." | taxon == "Oligochaeta indet" | taxon == "Potamophylax" | taxon == "Tub. 2"){
        for (d in 1:length(t)){
          date.sub = growth[which(growth$JULIAN == t[d]),]
          date.samps <- date.sub[sample(nrow(date.sub),1, replace = T),6:dim(date.sub)[2]]
          x = as.numeric(nrow(date.samps))
      ...
      list(totdays=sum(t.int), Psampest=Psampest, Pbootest=c(Mean=mean(Pboots), quantile(Pboots, c(0.025, 0.5, 0.975))), Pboots=Pboots, Bboots=Bboots, Nboots=Nboots, julians=t, intdays=t.int, Pintsampest=Pintsampest, Pintboots=Pintboots, Bdatesampinfo=Bdatesampinfo, Bintboots = Bintboots, Ndatesampinfo=Ndatesampinfo, Bdateboots=Bdateboots, Ndateboots=Ndateboots)
    }
