...
#Define IGR production bootstrap function:
  igr.int.prod <- function(DATA, GROWTH_df, site, first.date, last.date, habitat, taxon, TEMP, wrap=F, LM.a, LM.b, LM.p.ash, g.a, g.b, g.c, g.d, min.growth=0.001, temp.corr, boot.num){
...
print(taxon)
      ...
      for (d in 1:length(t)){
        ...
        N[d,] <- apply(rows, 2, na.rm_mean)											#For each date, compute the mean abundance from the selected samples for each size class
      ...
      } else if (taxon == "Tubificid 1" | taxon == "Tub. 2"| taxon == "Simulium vittatum" | taxon == "Simulium indet" |taxon == "Ostracoda" | taxon == "Oribatid 1" | taxon == "Leech" | taxon == "Nematoda" | taxon == "Sperchon sp."| taxon == "Sperchon glandulosus" | taxon == "Lumb 3" | taxon == "Springtail" | taxon =="Clinocera stagnalis" |
               taxon == "Antocha sp." | taxon == "Oribatid 2" |taxon == "Ephydridae sp." | taxon == "Oligochaeta indet." | taxon == "Potamophylax cingulatus" | taxon == "Tub. 2" | taxon == "Prosimulium ursinum" | taxon == "Simulium vernum" | taxon == "Simulium aureum" | taxon == "Oribatid 3" | taxon == "Red Mite"| taxon == "Lumbricidae"){
        for (d in 1:length(t)){
          date.sub = growth[which(growth$JULIAN == t[d]),]
          date.samps <- date.sub[sample(nrow(date.sub),1, replace = T),5:dim(date.sub)[2]]
          x = as.numeric(nrow(date.samps))
        ...
        for (d in 1:length(t)){
          ...
          for (s in 1:length(sizes$mm)){
            for (f in 1:length(t.int)){
              if (!is.null(x) && x != 0 && !is.na(date.samps[1,s])){
                int.P[f,s] = int.B[f,s]*t.int[f]*date.samps[1,s]
              } else if(10^(g.a + (g.b*sizes$AFDM[s])+ (g.c * overkt_to_C(TEMP[f]))) > 0){
                    int.P[f,s] <- int.B[f,s]*t.int[f]*(10^(g.a + (g.b*sizes$AFDM[s])+ (g.c * overkt_to_C(TEMP[f]))))		#Calculate interval production (interval biomass * interval duration * growth rate for each size class on each date * temp correction factor for each date)
                  }
                  else {
                    int.P[f,s] <- int.B[f,s]*t.int[f]*min.growth			#Calculate interval production (interval biomass * interval duration * minimum growth rate if growth equation gives a zero or negative value)
                  }
      ...
      for (b in 1:boot.num){
        ...
        } else if (taxon == "Tubificid 1" | taxon == "Tub. 2" | taxon == "Simulium vittatum" | taxon == "Simulium indet" |taxon == "Ostracoda" | taxon == "Oribatid 1" | taxon == "Leech" | taxon == "Nematoda" | taxon == "Sperchon glandulosus" | taxon == "Lumb 3" | taxon == "Springtail" | taxon =="Clinocera stagnalis" |
                 taxon == "Antocha sp." | taxon == "Oribatid 2" |taxon == "Ephydridae sp." | taxon == "Oligochaeta indet." | taxon == "Potamophylax cingulatus" | taxon == "Tub. 2" | taxon == "Prosimulium ursinum" | taxon == "Simulium vernum" | taxon == "Simulium aureum"| taxon == "Oribatid 3" | taxon == "Red Mite" | taxon == "Lumbricidae"){
          for (d in 1:length(t)){
            date.sub = growth[which(growth$JULIAN == t[d]),]
            date.samps <- date.sub[sample(nrow(date.sub),1, replace = T),5:dim(date.sub)[2]]
      ...
      list(totdays=sum(t.int), Psampest=Psampest, Pbootest=c(Mean=mean(Pboots), quantile(Pboots, c(0.025, 0.5, 0.975))), Pboots=Pboots, Bboots=Bboots, Nboots=Nboots, julians=t, intdays=t.int, Pintsampest=Pintsampest, Pintboots=Pintboots, Bintboots = Bintboots, Nintboots = Nintboots, Bdatesampinfo=Bdatesampinfo, Ndatesampinfo=Ndatesampinfo, Bdateboots=Bdateboots, Ndateboots=Ndateboots)
    }
