...
#Define IGR production bootstrap function:
  igr.int.prod <- function(DATA, GROWTH_df, site, first.date, last.date, habitat, taxon, TEMP, wrap=F, LM.a, LM.b, LM.p.ash, g.a, g.b, g.c, g.d, min.growth=0.0001, temp.corr, boot.num){
...
sizes <- data.frame(mm, AFDM)
      ...
      if (taxon == "Midge 1" | taxon == "Midge 2" | taxon == "Cricotopus sylvestris"){
        for (d in 1:length(t)){
          ...
          for (f in 1:length(t.int)){
            if (!is.null(x) && x != 0 && !is.na(date.samps[1,s])){
              ...
              int.P[f,s] <- int.B[f,s]*t.int[f]*(g.a-(g.b*log(sizes$mm[s]))+(g.c*overkt_to_C(TEMP[d])))	#Calculate interval production (interval biomass * interval duration * growth rate for each size class and temp on each date)
            }  else {
        ...
        for (s in 1:length(sizes$mm)){
          for (f in 1:length(t.int)){
            if (!is.null(x) && x != 0 && !is.na(date.samps[1,s])){
              ...
              int.P[f,s] <- int.B[f,s]*t.int[f]*(exp((g.a-(g.b*log(sizes$AFDM[s]))+(g.c*TEMP[d]))))	#Calculate interval production (interval biomass * interval duration * growth rate for each size class and temp on each date)
            }  else {
              int.P[f,s] <- int.B[f,s]*t.int[f]*min.growth	#Calculate interval production (interval biomass * interval duration * minimum growth rate if growth equation gives a zero or negative value)
            }
          }
        ...
        for (d in 1:length(t)){
          ...
          for (s in 1:length(sizes$mm)){
            for (f in 1:length(t.int)){
              if (!is.null(x) && x != 0 && !is.na(date.samps[1,s])){
                int.P[f,s] = int.B[f,s]*t.int[f]*date.samps[1,s]
              } else if (exp(g.a-(g.b*log(sizes$AFDM[s]))+(g.c*TEMP[d])) > 0 |!is.na(exp(g.a-(g.b*log(sizes$AFDM[s]))+(g.c*TEMP[d])))){
              int.P[f,s] <- int.B[f,s]*t.int[f]*(exp(g.a-(g.b*log(sizes$AFDM[s]))+(g.c*TEMP[d])))	#Calculate interval production (interval biomass * interval duration * growth rate for each size class on each date * temp correction factor for each date)
            }
            else {
              int.P[f,s] <- int.B[f,s]*t.int[f]*min.growth			#Calculate interval production (interval biomass * interval duration * minimum growth rate if growth equation gives a zero or negative value)
            }
      ...
      } else if (taxon == "Tub. 1" | taxon == "Simulium vittatum" | taxon == "Ostracoda" | taxon == "Oribatid 1" | taxon == "Leech" | taxon == "Nematoda" | taxon == "Sperchon sp." | taxon == "Lumb 3" | taxon == "Springtail" | taxon =="Clinocera stagnalis" | taxon == "Sperchon glandulosus" |
                 taxon == "Tubificid 1"){
        for (d in 1:length(t)){
          date.sub = growth[which(growth$JULIAN == t[d]),]
          date.samps <- date.sub[sample(nrow(date.sub),1, replace = T),5:dim(date.sub)[2]]
          ...
          for (s in 1:length(sizes$mm)){
            for (f in 1:length(t.int)){
              if (!is.null(x) && x != 0 && !is.na(date.samps[1,s])){
                int.P[f,s] = int.B[f,s]*t.int[f]*date.samps[1,s]
              } else if (exp(g.a + (g.b*log(sizes$AFDM[s])) + (g.c * TEMP[d])) > 0){
              int.P[f,s] <- int.B[f,s]*t.int[f]*(exp((g.a-(g.b*log(sizes$AFDM[s]))+(g.c*TEMP[d]))))		#Calculate interval production (interval biomass * interval duration * growth rate for each size class on each date * temp correction factor for each date)
            }
            else {
              int.P[f,s] <- int.B[f,s]*t.int[f]*min.growth			#Calculate interval production (interval biomass * interval duration * minimum growth rate if growth equation gives a zero or negative value)
      ...
      for (b in 1:boot.num){
          ...
          for (d in 1:length(t)){
            ...
            for (s in 1:length(sizes$mm)){
              for (f in 1:length(t.int)){
                if (!is.null(x) && x != 0 && !is.na(date.samps[1,s])){
                  int.P[f,s] = int.B[f,s]*t.int[f]*date.samps[1,s]
                } else if(exp(g.a-(g.b*log(sizes$mm[s]))+(g.c*TEMP[f])) > 0){
                  int.P[f,s] <- int.B[f,s]*t.int[f]*(exp((g.a-(g.b*log(sizes$AFDM[s]))+(g.c*TEMP[d]))))	#Calculate interval production (interval biomass * interval duration * growth rate for each size class and temp on each date)
                }  else {
                  int.P[f,s] <- int.B[f,s]*t.int[f]*min.growth	#Calculate interval production (interval biomass * interval duration * minimum growth rate if growth equation gives a zero or negative value)
                }
        ...
        } else if (taxon == "Tub. 1" | taxon == "Simulium vittatum" | taxon == "Ostracoda" | taxon == "Oribatid 1" | taxon == "Leech" | taxon == "Nematoda" | taxon == "Sperchon sp." | taxon == "Lumb 3" | taxon == "Springtail" | taxon =="Clinocera" | taxon == "Sperchon glandulosus" |
                   taxon == "Tubificid 1"){
          for (d in 1:length(t)){
            date.sub = growth[which(growth$JULIAN == t[d]),]
            date.samps <- date.sub[sample(nrow(date.sub),1, replace = T),5:dim(date.sub)[2]]
            ...
            for (s in 1:length(sizes$mm)){
              for (f in 1:length(t.int)){
                if (!is.null(x) && x != 0 && !is.na(date.samps[1,s])){
                  int.P[f,s] = int.B[f,s]*t.int[f]*date.samps[1,s]
                } else if (exp((g.a + (g.b*sizes$AFDM[s])) + (g.c * TEMP[d])) > 0){
                  int.P[f,s] <- int.B[f,s]*t.int[f]*(exp((g.a-(g.b*log(sizes$AFDM[s]))+(g.c*TEMP[d]))))		#Calculate interval production (interval biomass * interval duration * growth rate for each size class on each date * temp correction factor for each date)
                }
                else {
                  int.P[f,s] <- int.B[f,s]*t.int[f]*min.growth			#Calculate interval production (interval biomass * interval duration * minimum growth rate if growth equation gives a zero or negative value)
