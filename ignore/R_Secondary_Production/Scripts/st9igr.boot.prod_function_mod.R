...
print(taxon)
      ...
      if (taxon == "Micropsectra sp." | taxon == "Eukiefferiella" | taxon == "Diamesa bohemani_zernyi" | taxon == "Midge 3-2" | taxon == "ST9 Midge 4" | taxon == "Orthocladius fridgidus" | taxon == "Ceratopegonid" |taxon == "Thaumaleidae" |
          taxon == "Midge 4" | taxon == "Midge 5" | taxon == "Midge 6" | taxon == "Midge 7" | taxon == "Midge 8" | taxon == "Orthocladius spp." | taxon == "Thienemanniella sp." | taxon == "Midge indet." | taxon == "Rheocricotopus effusus" |
          taxon == "Eukiefferiella sp." | taxon == "Macropelopia" | taxon == "Midge B" | taxon == "Midge C" | taxon == "Midge D" | taxon == "Midge E"| taxon == "Parochlus sp."| taxon == "Orthocladius oblidens" |
          taxon == "Thysanoptera" | taxon == "Thrips"){
        for (d in 1:length(t)){
        ...
        for (d in 1:length(t)){
          ...
          for (s in 1:length(sizes$mm)){
            for (f in 1:length(t.int)){
              ...
              int.P[f,s] <- int.B[f,s]*t.int[f]*(g.a + (g.c*sizes$AFDM[s])+ (g.b * TEMP[f]))		#Calculate interval production (interval biomass * interval duration * growth rate for each size class on each date * temp correction factor for each date)
            }
            else {
              int.P[f,s] <- int.B[f,s]*t.int[f]*min.growth	#Calculate interval production (interval biomass * interval duration * minimum growth rate if growth equation gives a zero or negative value)
            }
        ...
        for (d in 1:length(t)){
          ...
          for (s in 1:length(sizes$mm)){
            for (f in 1:length(t.int)){
              if (!is.null(x) && x != 0 && !is.na(date.samps[1,s])){
                int.P[f,s] = int.B[f,s]*t.int[f]*date.samps[1,s]
              } else if(exp(g.a + (g.b*log(sizes$AFDM[s])) + (g.c*TEMP[f])) > 0 |!is.na(exp(g.a + (g.b*TEMP[f]) + (g.c*log(sizes$AFDM[s]))))){
                int.P[f,s] <- int.B[f,s]*t.int[f]*(exp(g.a + (g.b*log(sizes$AFDM[s])) + (g.c*TEMP[f])))	#Calculate interval production (interval biomass * interval duration * growth rate for each size class on each date * temp correction factor for each date)
            }
            else {
              int.P[f,s] <- int.B[f,s]*t.int[f]*min.growth			#Calculate interval production (interval biomass * interval duration * minimum growth rate if growth equation gives a zero or negative value)
            }
      ...
      } else if (taxon == "Tubificid 1" | taxon == "Tub. 2"| taxon == "Simulium vittatum" | taxon == "Simulium spp." |taxon == "Ostracoda" | taxon == "Oribatid 1" | taxon == "Leech" | taxon == "Nematoda" | taxon == "Sperchon glandulosus" | taxon == "Lumb 3" | taxon == "Springtail" | taxon =="Clinocera stagnalis" |
               taxon == "Antocha sp." | taxon == "Oribatid 2" | taxon == "Oribatid 3" |taxon == "Ephydridae sp." | taxon == "Oligochaeta indet." | taxon == "Potamophylax cingulatus" | taxon == "Tub. 2" | taxon == "Prosimulium ursinum" | taxon == "Simulium vernum" | taxon == "Simulium aureum" | taxon == "Lumbricidae"){
        for (d in 1:length(t)){
          date.sub = growth[which(growth$JULIAN == t[d]),]
          date.samps <- date.sub[sample(nrow(date.sub),1, replace = T),5:dim(date.sub)[2]]
          x = as.numeric(nrow(date.samps))
          for (s in 1:length(sizes$mm)){
            for (f in 1:length(t.int)){
              if (!is.null(x) && x != 0 && !is.na(date.samps[1,s])){
                int.P[f,s] = int.B[f,s]*t.int[f]*date.samps[1,s]
              } else if((g.a + (g.b*sizes$AFDM[s]) + (g.c * TEMP[f])) > 0){
              int.P[f,s] <- int.B[f,s]*t.int[f]*(g.a + (g.b*sizes$AFDM[s]) + (g.c * TEMP[f]))		#Calculate interval production (interval biomass * interval duration * growth rate for each size class on each date * temp correction factor for each date)
            }
            else {
              int.P[f,s] <- int.B[f,s]*t.int[f]*min.growth			#Calculate interval production (interval biomass * interval duration * minimum growth rate if growth equation gives a zero or negative value)
            }
      ...
      }      else if (taxon == "Nais spp." | taxon == "Hydra"){
        for (d in 1:length(t)){
          date.sub = growth[which(growth$JULIAN == t[d]),]
          date.samps <- date.sub[sample(nrow(date.sub),1, replace = T),5:dim(date.sub)[2]]
          x = as.numeric(nrow(date.samps))
