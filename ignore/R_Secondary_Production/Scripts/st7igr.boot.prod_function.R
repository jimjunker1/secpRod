  ...
  igr.int.prod <- function(DATA, site, first.date, last.date, habitat, taxon, TEMP, wrap=F, LM.a, LM.b, LM.p.ash, g.a, g.b, g.c, g.d, min.growth=0, temp.corr, boot.num, GROWTH_df,...){
    ...
    if (length(TEMP) == length(t.int) & length(temp.corr) == length(t.int)){
      ...
      if (taxon == "Ceratopogonidae A" | taxon == "Ceratopogonidae B" | taxon == "Macropelopia" | taxon == "Eukiefferiella minor" | taxon == "Eukiefferiella claripennis" | taxon == "Diamesa bohemani-zernyi" |
            taxon == "Diamesa bertrami" | taxon == "Micropsectra" | taxon == "Parochlus" | taxon == "Thienemaniella" | taxon == "Thienemaniella sp."|taxon == "Orthocladius frigidus" | taxon == "Orthocladius oblidens" | taxon == "Metriocnemus" |
