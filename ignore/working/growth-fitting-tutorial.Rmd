...
3) returns a probability distribution that can integrate into downstream production methods used in the package
...
4) Compare the distribution of estimates of growth to our analytically calculated rates

### Simulate growth and length-frequency data {-}

#### Gompertz model {-}

First, we will simulate varying individual growth data with known parameters to create length-frequency distributions from which we will attempt to recover growth parameters. We will use a Gompertz growth model [@winsor1932] as:
