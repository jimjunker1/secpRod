  ...
  igr.int.prod <- function(DATA, GROWTH_df, site, first.date, last.date, habitat, taxon, TEMP, wrap=F, LM.a, LM.b, LM.p.ash, g.a, g.b, g.c, g.d, min.growth=0.001, temp.corr, boot.num){
    ...
    if (length(TEMP) == length(t.int) & length(temp.corr) == length(t.int)){
      ...
      for (b in 1:boot.num){
        ...
        int.P <- matrix(0, length(t.int), length(sizes$mm))#ditch this
          ...
          for (s in 1:length(sizes$mm)){
            for (f in 1:length(t.int)){
              ...
              int.P[f,s] <- int.B[f,s]*t.int[f]*min.growth			#Calculate interval production (interval biomass * interval duration * minimum growth rate if growth equation gives a zero or negative value)
               ...
               taxon == "Antocha sp." | taxon == "Oribatid 2" |taxon == "Ephydridae sp." | taxon == "Oligochaeta indet." | taxon == "Potamophylax" | taxon == "Tub. 2" | taxon == "Prosimulium" | taxon == "S. vernum" | taxon == "S. aurem" | taxon == "Oribatid 3" | taxon == "Red Mite"){
        for (d in 1:length(t)){#Can ditch this for loop throughout
          date.sub = growth[which(growth$JULIAN == t[d]),]
          date.samps <- date.sub[sample(nrow(date.sub),1, replace = T),6:dim(date.sub)[2]]
          x = as.numeric(nrow(date.samps))
          for (s in 1:length(sizes$mm)){
      ...
      list(totdays=sum(t.int), Psampest=Psampest, Pbootest=c(Mean=mean(Pboots), quantile(Pboots, c(0.025, 0.5, 0.975))), Pboots=Pboots, Bboots=Bboots, Nboots=Nboots, julians=t, intdays=t.int, Pintsampest=Pintsampest, Pintboots=Pintboots, Bintboots = Bintboots, Bdatesampinfo=Bdatesampinfo, Ndatesampinfo=Ndatesampinfo, Bdateboots=Bdateboots, Ndateboots=Ndateboots)
    }
