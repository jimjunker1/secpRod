...
- density: organism density in the replicate

- afdm_mg: a list-column of individual masses. This can be of arbitrary length. For example, it can contain measurements for every individual, as in the case of this simulated example, or be a representative sub-sample.
...
ggplot(summary_stats, aes(x = dateID)) +
  stat_halfeye(data = singleCohortSim, aes(x = dateID, y = density),
               color = 'green')+
  stat_halfeye(data = singleCohortSim %>% unnest(afdm_mg), aes(x = dateID, y = afdm_mg*100),
               color = 'red')+
