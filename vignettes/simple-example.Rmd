...
taxaInfo <- data.frame(
  taxonID = c("sppA"),
  massForm = "afdm_mg~(a*lengthClass^b)*percAsh",
  a = c(0.0025),
  b = c(2.692),
  percAsh = c(0.958),
  method = c("sf"),
  g.a = c(NA),
  growthForm = "log(g_d)~0.05 - 0.25*log(afdm_mg) - 0.02*temp_C",
  min.cpi = c(335),
  max.cpi = c(365),
...
The univoltine example 

```{r univoltine-plot, echo = FALSE}

univoltine %>%
  convert_length_to_mass(taxaSampleList = ., taxaInfo = taxaInfo) %>% 
  plot_cohorts(taxaSampleListMass = .,
             param = 'mass',
             massClass = 'afdm_mg',
             # choose a vertical violin plot
             type = 'v')
