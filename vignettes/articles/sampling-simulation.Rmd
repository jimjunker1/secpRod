...
We developed a quick algorithm to automate the determination of the beginning and ends of a *non-overlapping* cohort, `detect_cohort_bounds()`

```{r detect-cohort-bounds}

df <- splitCohortSim %>% mutate(repID = 1:n(), .by = c('day')) %>% 
  select(dateID = day, repID, n_m2 = larvalDensity, massValue = massDistribution) %>% 
  unnest(massValue)

fit_with_offset <- function(df_ordered, offset, models_to_fit = c("vbg", "gompertz", "logistic", "richards"), t_start = 5) {
  stopifnot("cohort" %in% names(df_ordered))

  # Split cohorts
  youngest_dates <- df_ordered %>%
    filter(cohort == min(cohort)) %>%
    arrange(dateID)

  oldest_dates <- df_ordered %>%
  ...
  aiccs <- c()
   ...
   # von Bertalanffy
  if ("vbg" %in% models_to_fit) {
    m <- try(nls(W_all ~ Winf*(1-exp(-k*(t_all - t0))),
      start=list(Winf=max(W_all),k=0.01,t0=t_start)), silent=TRUE)
...
reconstruct_cohort_pseudotime <- function(df,
                                                       ...
                                                       t_start = 5,
                                                       models_to_fit = c("vbg", "gompertz", "logistic", "richards"),
...
x = reconstruct_cohort_pseudotime(df = df)
```

```{r plot-remap}
plot_cohort_fit <- function(remap_result, model = "ensemble", label_points = TRUE) {
  require(ggplot2)
  require(dplyr)
  ...
  model_names <- names(remap_result$fits)
  ...
  ggplot(pred_df, aes(x = pseudotime)) +
    ...
    {if (label_points) geom_text(aes(label = df$dateID, y = observed_mass), vjust = -1, size = 3)}
}

# Model-averaged fit
# debugonce(plot_cohort_fit)
plot_cohort_fit(x, model = "ensemble")

# Specific model fit
plot_cohort_fit(x, model = "vbg")
plot_cohort_fit(x, model = "gompertz", label_points = FALSE)
plot_cohort_fit(x, model = "logistic")
...
fit_with_offset <- function(df_ordered, offset, models_to_fit = c("vonbert", "gompertz"), t_start = 5) {
  ...
  youngest_dates <- df_ordered %>% filter(cohort == min(cohort)) %>% arrange(dateID)
  delta_young <- diff(youngest_dates$dateID)
  young_pseudotime <- c(0, cumsum(delta_young))

  oldest_dates <- df_ordered %>% filter(cohort == max(cohort)) %>% arrange(dateID)
  delta_old <- diff(oldest_dates$dateID)
...
debugonce(fit_with_offset)
  ...
  df_final <- df_ordered %>%
    mutate(pseudotime = pseudo_time_shifted) %>%
    arrange(pseudotime)
