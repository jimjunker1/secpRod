...
ggplot(summaryStats, aes(x = dateID)) +
  stat_halfeye(data = singleCohortSim, aes(x = dateID, y = density),
               color = 'green')+
  stat_halfeye(data = singleCohortSim %>% unnest(mass), aes(x = dateID, y = mass*100),
               color = 'red')+
  geom_line(aes(y = larvalDensityMean), color = 'green') +
  geom_line(aes(y = massMean * 100), color = 'red') +
  scale_y_continuous(
    name = "Larval Density",
    sec.axis = sec_axis(~./100, name = "Mean Mass (AFDM mg)")
  ) +
  theme_minimal() +
...
ggplot(summary_stats, aes(x = dateID)) +
  stat_halfeye(data = splitCohortSim, aes(x = dateID, y = density),
               color = 'green')+
  stat_halfeye(data = splitCohortSim %>% unnest(mass), aes(x = dateID, y = mass*100),
               color = 'red')+
...
remappedCohort = reconstruct_split_cohort(df = splitCohortSim %>% unnest(mass, keep_empty = TRUE),
                                          timeCol = "dateID",
                                          massCol = "mass",
                                          massDropThresh = 0.6,
                                          tStart = 5,
                                          models = c("vbg", "gompertz", "logistic"),
...
```{r plot-remap, out.width='75%', dpi=300, fig.align='center'}
